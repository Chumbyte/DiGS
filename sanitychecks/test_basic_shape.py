...
model_name, n_loss_type, normalize_normal_loss, unsigned_n, unsigned_d, loss_type, seed, encoder_type,\
    ...
    args.logdir, args.model_name, args.n_loss_type, \
...
# get loss
criterion = DiGSNet.DiGSLoss(weights=[3e3, 1e2, 1e2, 5e1, 1e1], #sdf, intern, normal, eikonal, div
...
for epoch in args.epoch_n:
    model_filename = os.path.join(model_dir, '%s_model_%d.pth' % (model_name, epoch))
